...
def main():
    ...
    with st.sidebar:
        ...
        sel = [name for name in NSE_INDEX_CSVS
               if st.checkbox(name, value=True, key=f"u_{name}")]

        st.markdown("<div class='sb-h'>Scan day</div>", unsafe_allow_html=True)
        scan_date = st.date_input("Candle date", value=dt.date.today(),
                                  help="Scans the last trading day on or before this date.",
                                  label_visibility="collapsed")

        st.markdown("<div class='sb-h'>Candle definition</div>", unsafe_allow_html=True)
        touch_label = st.radio("Wick must reach", ["Top of EMA band", "Below both EMAs"],
                               help="‘Top of band’ is looser and catches more names. "
                                    "‘Below both’ is the strict version.")
        touch_mode = "both" if touch_label == "Below both EMAs" else "band_top"
        tol = st.slider("Touch tolerance %", 0.0, 1.0, 0.20, 0.05,
                        help="Cushion so a wick that falls just short still counts.") / 100.0
        require_bullish = st.checkbox("Candle must be green (Close > Open)", value=True)

        st.markdown("<div class='sb-h'>&nbsp;</div>", unsafe_allow_html=True)
        run = st.button("⬇  Run scan", type="primary")

    # ── Run scan: fetch prices ────────────────────────────────────────────────
    if run:
        if not sel:
            ...
            with st.status("Loading NSE universe…", expanded=False) as status:
                ...
                if not universe:
                    ...
                    st.error("Couldn't load any symbols from NSE. Check your internet "
                             "(this must run on your machine, not in the sandbox).")
                    st.stop()
    ...
    # ── Legend ────────────────────────────────────────────────────────────────
    st.markdown(
        "<div class='legend'><b>The candle:</b> the lower wick taps the EMA10/EMA20 band and "
        "the candle closes above <b>both</b> EMAs (a green reclaim). &nbsp;|&nbsp; "
        "<b>The four checks</b> (shown per stock, they don’t filter anything out): "
        "RSI&nbsp;&gt;&nbsp;60 &middot; MACD&nbsp;&gt;&nbsp;Signal &middot; MACD&nbsp;&gt;&nbsp;0 &middot; "
        "Volume&nbsp;&gt;&nbsp;20-day average.</div>", unsafe_allow_html=True)

    # ── Results ───────────────────────────────────────────────────────────────
    prices = st.session_state.prices
    if prices is None:
        st.markdown("<div class='empty'><div class='big'>No scan yet</div>"
                    "Pick your universe and candle settings on the left, then "
                    "<b>Run scan</b>.</div>", unsafe_allow_html=True)
        return
    if not prices:
        st.markdown("<div class='empty'><div class='big'>Couldn’t fetch any prices</div>"
                    "Check your connection and run this on your own machine.</div>",
    ...
    if not results:
        st.markdown("<div class='empty'><div class='big'>No stocks printed this candle "
                    "on the chosen day</div>Try loosening the touch tolerance or switching "
                    "to ‘Top of EMA band’.</div>", unsafe_allow_html=True)
        return
    ...
    with c2:
        sort_by = st.selectbox("Sort by", ["Conditions met", "RSI", "Volume ratio", "Symbol A–Z"],
                               label_visibility="collapsed")
    df = pd.DataFrame(results)
    if view == "Pass all 4 only":
        df = df[df["AllPass"]]
    sort_map = {"Conditions met": (["Met", "RSI"], [False, False]),
                "RSI": (["RSI"], [False]), "Volume ratio": (["VolRatio"], [False]),
                "Symbol A–Z": (["Symbol"], [True])}
    cols, asc = sort_map[sort_by]
